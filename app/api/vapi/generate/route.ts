...
export async function POST(request: Request) {
  try {
    ...
    // Distinguish if it's an interview generation or feedback submission
    if (body.action === "generateInterview") {
      const { type, role, level, techstack, amount, userid } = body;
      ...
      const { text: questions } = await generateText({
        ...
        prompt: `
          ...
          Return ONLY questions in JSON array: ["Question 1", "Question 2", ...]
      ...
      console.log("Interview generated:", docRef.id);
      return Response.json({ success: true, interviewId: docRef.id });

    } else if (body.action === "submitFeedback") {
      const { interviewId, userId, score, remarks } = body;

      if (!interviewId || !userId || score === undefined || !remarks) {
        return Response.json({ success: false, message: "Missing feedback fields" }, { status: 400 });
      }
      ...
      return Response.json({ success: true, feedbackId: docRef.id });
    }
...
export async function GET(request: Request) {
  try {
    ...
    const { action, interviewId } = Object.fromEntries(searchParams);

    if (action === "getFeedback") {
      if (!interviewId) return Response.json({ success: false, message: "Missing interviewId" }, { status: 400 });
    ...
    return Response.json({ success: true, data: "API is live" });
  } catch (error) {
